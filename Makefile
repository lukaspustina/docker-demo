...
build:
	@docker build --no-cache --rm -t docker_demo/python python
	@docker build --no-cache --rm -t docker_demo/webserver webserver 
	@docker images

run: $(LOGDIR)
	@echo "+++ Starting containers +++"
	@for i in `seq 1 $(NUM_OF_SERVERS)`; do \
		docker run -d --cidfile=$</webserver-$$i.cid --name webserver-$$i -v `pwd`/$<:/logs docker_demo/webserver:latest /opt/webserver/run.sh /logs ; \
	done
